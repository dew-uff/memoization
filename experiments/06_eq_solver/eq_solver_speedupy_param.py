...
@deterministic
def solve(n, F_solution, func_globals=None):
    T = 1
    C = 1
    Kd1 = 2
    n2 = 1000
...
@deterministic
def exp_main(F_solution, n_solutions, func_globals=None):
    x_list = np.linspace(1, 20000.0, n_solutions)
    F_list = np.empty_like(x_list)
    for i in range(n_solutions):
        F_solution = solve(x_list[i], F_solution, func_globals=globals())
        F_list[i] = F_solution
