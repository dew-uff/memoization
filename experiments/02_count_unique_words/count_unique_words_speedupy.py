...
@deterministic
def strip_word(word, func_globals=None):
    temp1 = ''
...
def count_words_dictionary(file_name):
    ...
    data = temp2.read()
    return process_count_words_dictionary(data, func_globals=globals())

@deterministic
def process_count_words_dictionary(data, func_globals=None):
    dictionary = defaultdict(int)
    for word in data.split():
        dictionary[strip_word(word, func_globals=globals())] += 1
    del dictionary['']
...
def count_words_set(file_name):
    with open(file_name, 'r') as file_id:
        data = file_id.read()
        return process_count_words_set(data, func_globals=globals())

@deterministic
def process_count_words_set(data, func_globals=None):
    lines = data.splitlines()
    uniques = set()
    for line in lines:
        uniques |= set((strip_word(m, func_globals=globals()) for m in line.split()))
    uniques.remove('')
    return len(uniques)
if len(sys.argv) < 1:
    print('Usage:')
